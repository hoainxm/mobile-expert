    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                ...
                console.error('Lỗi khi lấy danh sách đơn hàng:', error.message || error);
            ...
            <FlatList
                ...
                renderItem={({ item }: { item: any }) => (
                    <View style={styles.productCard}>
                        <Image
                            source={{ uri: item.productId?.image || 'https://via.placeholder.com/50' }}
                            style={styles.productImage}
                        />
                        <View>
                            <Text style={styles.productName}>{item.productId?.name || 'Sản phẩm không xác định'}</Text>
                            <Text style={styles.productQuantity}>Số lượng: {item.quantity}</Text>
                        </View>
                    </View>
                )}
                keyExtractor={(product) => product.productId?._id || Math.random().toString()}
                horizontal
