...
const OrderDetailsScreen: React.FC<{ route: any }> = ({ route }) => {
    ...
    const accesstoken = useSelector((state: any) => state.authReducer.authData.accesstoken);
    ...
    const handleAddReview = async () => {
        ...
        try {
            ...
            Alert.alert('Thành công', response.message || 'Đánh giá thành công!');
            setRating(0);
            setComment('');
            setSelectedProductId(null);
        } catch (error: any) {
            console.error('Lỗi khi thêm đánh giá:', error.response?.data || error.message);
            Alert.alert('Lỗi', error.response?.data?.message || 'Không thể thêm đánh giá. Vui lòng thử lại sau.');
        }
    };
