...
const HomeScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    const fetchTopSellingProducts = async () => {
      try {
        ...
        const result = await axiosClient.get<Product[]>('/products/top-selling');
        setProducts(result || []);
      } catch (error: any) {
  ...
  const renderItem = ({ item }: { item: Product }) => (
    <View style={styles.productCard}>
      ...
      >
        <MaterialIcons
          name={favoriteProducts.find((fav) => fav._id === item._id) ? 'favorite' : 'favorite-border'}
          size={24}
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={() => navigation.navigate('Favorites')}>
          <MaterialIcons name="favorite" size={28} color={appColors.danger} />
        </TouchableOpacity>
