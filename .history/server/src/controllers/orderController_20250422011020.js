...
exports.getOrderByUserIdStatistics = async (req, res) => {
  try {
    ...
    const stats = await Order.aggregate([
      {
        // Stage 1: Match orders for the specific user with 'Completed' status
        $match: {
          userId: userObjectId,
      ...
      {
        // Stage 2: Group the matched documents to calculate totals
        $group: {
      ...
      {
         // Stage 3: Project to reshape the output (optional but good practice)
         $project: {
    ...
    const result = stats[0] || { completedOrdersCount: 0, totalCompletedAmount: 0 };
